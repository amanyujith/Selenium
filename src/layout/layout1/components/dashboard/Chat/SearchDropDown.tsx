...
const SearchDropDown = (props: any) => {
  ...
  const handlesearch = () => {
    if (searchText !== "") {
      ...
      searchActiveChat
        ? navigate(`${path.SEARCHRESULT}/${searchText}/${activeChat.uuid}`)
        : navigate(`${path.SEARCHRESULT}/${searchText}`)
      closeSearch()
    }
  }
  ...
  const handleClick = (data: any) => {
    if (!notification) {
      if (data.type === "group") {
        ...
        const hasUser = usersList.some((node: any) => node.uuid === data.uuid)
      ...
      if (updateSettings) {
        const targets = settings[0].target.map((user: any) => {
          return user.uuid
        })
        updateSettings({
          target: [...targets, data.uuid].reverse(),
        })
        setSearchText("")
      }
      setSearchBox("")
  ...
  return (
    <div
      ...
      className=" w-[402px] h-fit shadow-[0_10px_15px_-5px_rgba(0,0,0,0.1),0_10px_15px_-5px_rgba(0,0,0,0.04)] bg-[#ffffff] rounded-b-[15px]"
    >
      <div className="h-fit w-full px-5 pt-2">
        <div>
          <div
          ...
          >
            {!notification && (
              <div
                ...
                className="flex flex-row h-fit ml-1 py-2 px-3 cursor-pointer text-[#9f9d9d] hover:bg-[#0000001f]"
              >
                Search
                <div className="ml-3 text-[#9f9d9d] text-base">
                  "{searchText}"
                </div>
              </div>
            )}
                ...
                {userList.map((item: any) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {item.type === "group" && item.status === "archive" && (
