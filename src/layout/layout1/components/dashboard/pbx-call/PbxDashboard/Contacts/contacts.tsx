...
import { IChatRoot, IUserData } from "../../../Chat/interfaces"
import ContactList from "./contactList"
import { motion } from "framer-motion"
import { contact_section_icon } from "../../vault/svg"
...
const Contacts = ({ initiateCall }: contacts) => {
  ...
  useEffect(() => {
    if (!activeContact?.id)
      dispatch(actionCreators.setActiveContact(contactList[0]?.id))
  }, [contactList])

  return (
    <div className="flex h-screen mr-3 origin-top ">
      <motion.div
        key="contactlist"
        ...
        className=" min-w-[339px] h-[calc(100vh-130px)] rounded-xl mb-[24px] bg-[#ffffff]"
      >
        <div className="mt-[16px] mx-[20px] font-semibold text-[16px] font-sans flex justify-between">
          <span>Contacts</span>
          {/* {contact_section_icon} */}
        </div>
        <div onClick={(e: any) => {}}>
          <div className="h-11 flex items-center mt-2 mb-3 mx-3 py-2.5 pl-3.5 border-[1px] bg-[#ffffff14] border-[#0000001f] border-solid rounded-[10px]">
            <svg
              width="18"
              height="18"
              viewBox="0 0 18 18"
      ...
      <ContactInfo initiateCall={initiateCall} />
    </div>
  )
}
